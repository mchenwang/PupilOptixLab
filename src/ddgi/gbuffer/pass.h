...
#include "ddgi/gbuffer/type.h"

#include <memory>

struct GBufferPassSBTTypes {
...
struct GBufferPass {
    ...
    std::unique_ptr<optix_wrap::Pass<GBufferPassSBTTypes, GBufferPassOptixLaunchParams>> pass;
    GBufferPassOptixLaunchParams params;
    ...
    GBufferPass(device::Optix *device) noexcept {
        optix_device = device;
        pass = std::make_unique<optix_wrap::Pass<GBufferPassSBTTypes, GBufferPassOptixLaunchParams>>(optix_device->context, optix_device->cuda_stream);
        ConfigPipeline();
    }
    ...
    void Run() noexcept {
        ...
        pass->Run(params, params.frame.width, params.frame.height);
        ++params.frame_cnt;
    }
    ...
    void ConfigPipeline() noexcept {
        ...
        pt_module = std::make_unique<optix_wrap::Module>(optix_device->context, "ddgi/gbuffer/gbuffer_main.ptx");
    ...
    void ConfigSBT(scene::Scene *scene) noexcept {
        optix_wrap::SBTDesc<GBufferPassSBTTypes> desc{};
        desc.ray_gen_data = {
            .program_name = "__raygen__main",
            .data = GBufferPassSBTTypes::RayGenDataType{}
        };
        {
            int emitter_index_offset = 0;
            using HitGroupDataRecord = decltype(desc)::Pair<GBufferPassSBTTypes::HitGroupDataType>;
        ...
        {
            decltype(desc)::Pair<GBufferPassSBTTypes::MissDataType> miss_data = {
                .program_name = "__miss__default",
                .data = GBufferPassSBTTypes::MissDataType{}
            };
            desc.miss_datas.push_back(miss_data);
            decltype(desc)::Pair<GBufferPassSBTTypes::MissDataType> miss_shadow_data = {
    ...
    void InitLaunchParams(scene::Scene *scene) {
        params.frame.width = scene->sensor.film.w;
        params.frame.height = scene->sensor.film.h;
        params.frame_cnt = 0;

        CUDA_CHECK(cudaMalloc(
            reinterpret_cast<void **>(&params.albedo_buffer),
            params.frame.height * params.frame.width * sizeof(float4)));
        CUDA_CHECK(cudaMalloc(
            reinterpret_cast<void **>(&params.normal_buffer),
            params.frame.height * params.frame.width * sizeof(float4)));
