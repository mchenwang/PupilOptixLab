...
void Geometry::LoadGeometry(const scene::Shape &shape) noexcept {
    switch (shape.type) {
        case scene::EShapeType::_cube: {
            ...
            this->tri_mesh.positions.SetData(GetCudaMemory(shape.cube.positions, position_size));
            auto normals_size = shape.cube.vertex_num * 3 * sizeof(float);
            this->tri_mesh.normals.SetData(GetCudaMemory(shape.cube.normals, normals_size));
            auto tex_size = shape.cube.vertex_num * 2 * sizeof(float);
            this->tri_mesh.texcoords.SetData(GetCudaMemory(shape.cube.texcoords, tex_size));
            auto idx_size = shape.cube.face_num * 3 * sizeof(uint32_t);
            this->tri_mesh.indices.SetData(GetCudaMemory(shape.cube.indices, idx_size));
        } break;
        case scene::EShapeType::_obj: {
            ...
            this->tri_mesh.positions.SetData(GetCudaMemory(shape.obj.positions, position_size));
            auto normals_size = shape.obj.vertex_num * 3 * sizeof(float);
            this->tri_mesh.normals.SetData(GetCudaMemory(shape.obj.normals, normals_size));
            auto tex_size = shape.obj.vertex_num * 2 * sizeof(float);
            this->tri_mesh.texcoords.SetData(GetCudaMemory(shape.obj.texcoords, tex_size));
            auto idx_size = shape.obj.face_num * 3 * sizeof(uint32_t);
            this->tri_mesh.indices.SetData(GetCudaMemory(shape.obj.indices, idx_size));
        } break;
        case scene::EShapeType::_rectangle: {
            ...
            this->tri_mesh.positions.SetData(GetCudaMemory(shape.rect.positions, position_size));
            auto normals_size = shape.rect.vertex_num * 3 * sizeof(float);
            this->tri_mesh.normals.SetData(GetCudaMemory(shape.rect.normals, normals_size));
            auto tex_size = shape.rect.vertex_num * 2 * sizeof(float);
            this->tri_mesh.texcoords.SetData(GetCudaMemory(shape.rect.texcoords, tex_size));
            auto idx_size = shape.rect.face_num * 3 * sizeof(uint32_t);
            this->tri_mesh.indices.SetData(GetCudaMemory(shape.rect.indices, idx_size));
        } break;
