...
extern "C" __global__ void __raygen__main() {
    ...
    if (record.hit_flag) {
        auto pos = optix_launch_params.camera_proj_view_mat * make_float4(local_hit.geo.position, 1.f);

        pos = optix_launch_params.camera_view_mat * make_float4(local_hit.geo.position, 1.f);

        optix_launch_params.albedo[pixel_index] = make_float4(local_hit.mat.GetColor(local_hit.geo.texcoord), 1.f);
        optix_launch_params.normal[pixel_index] = make_float4(local_hit.geo.normal * 0.5f + 0.5f, 1.f);
        // optix_launch_params.depth[pixel_index] = make_float4(optix::ACESToneMapping(make_float3(pos), 1.f), 1.f);
        optix_launch_params.depth[pixel_index] = make_float4(make_float3(pos.z), 1.f);
    } else {
        optix_launch_params.albedo[pixel_index] = make_float4(0.f, 0.f, 0.f, 1.f);
        optix_launch_params.normal[pixel_index] = make_float4(0.f, 0.f, 0.f, 1.f);
        optix_launch_params.depth[pixel_index] = make_float4(0.f, 0.f, 0.f, 1.f);
    }

    // result += record.radiance;

    // if (optix_launch_params.sample_cnt > 0) {
    //     const float t = 1.f / (optix_launch_params.sample_cnt / optix_launch_params.spp + 1.f);
    //     const float3 pre = make_float3(optix_launch_params.accum_buffer[pixel_index]);
    //     result = lerp(pre, result, t);
    // }
    // optix_launch_params.accum_buffer[pixel_index] = make_float4(result, 1.f);
